...
class VMOrchestrator:
    ...
    async def create_template(
        ...
        asyncio.create_task(self._build_template(template))
    ...
    async def _build_template(self, template: VMTemplate):
        ...
        try:
            ...
            volume = self.libvirt.create_volume(pool, disk_name, template.disk_size_gb)
            ...
            logger.error(f"Failed to build template {template.name}: {e}")
    ...
    def _build_install_domain_xml(self, template: VMTemplate, disk_path: str) -> str:
        ...
        os_variant = os_variants.get(template.os_type, "generic")
    ...
    def _build_analysis_domain_xml(self, template: VMTemplate, disk_path: str, task: AnalysisTask) -> str:
        ...
        # Port for guest agent communication
        agent_port = self.agent_port + len(self.instances)
    ...
    async def revert_instance(self, instance_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to revert instance {instance_id}: {e}")
    ...
    async def _monitor_execution(self, instance: VMInstance, task: AnalysisTask):
        ...
        while time.time() - start < timeout:
            ...
            if time.time() - start > 30:  # Simulate 30s execution
                break
