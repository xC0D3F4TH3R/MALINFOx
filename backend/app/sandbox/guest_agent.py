...
class APICallMonitor:
    ...
    def _etw_loop(self):
        ...
        # This is a simplified version that logs common suspicious APIs
        suspicious_apis = {
            "kernel32.dll": [
            ...
            "ntdll.dll": [
                "NtCreateThreadEx", "NtWriteVirtualMemory", "NtAllocateVirtualMemory",
                "NtProtectVirtualMemory", "NtQueueApcThread", "NtSetInformationThread",
            ],
            "advapi32.dll": [
            ...
            "ws2_32.dll": [
                "connect", "send", "recv", "WSASocket", "WSAConnect",
            ],
            "wininet.dll": [
            ...
            class EventHandler(pyinotify.ProcessEvent):
                ...
                def _handle_event(self, event_type, event):
                    try:
                        ...
                        logger.error(f"File event error: {e}")
...
class RegistryMonitor:
        ...
        while self.running:
            for key_path in self.watched_keys:
                try:
                    ...
                    logger.debug(f"Registry check error for {key_path}: {e}")
    ...
    def _check_registry_key(self, key_path: str, reg_states: dict):
        ...
        try:
            ...
            logger.debug(f"Registry key error: {e}")
...
class SampleExecutor:
    ...
    def execute(self, sample_path: str, args: str = "") -> int:
        ...
        try:
            ...
            logger.info(f"Executing sample: {' '.join(cmd)}")
            ...
            logger.error(f"Sample execution error: {e}")
...
class GuestAgent:
    ...
    def start(self):
        ...
        # Start event flusher
        asyncio.create_task(self.collector.periodic_flush())
    ...
    def _connect_to_host(self):
        ...
        try:
            ...
            self.connected = True
            logger.info(f"Connected to host at {self.config.host_socket}")
...
async def main():
    ...
    try:
        ...
        if config.sample_path and os.path.exists(config.sample_path):
            if agent.sample_executor:
                pid = agent.sample_executor.execute(config.sample_path)
                logger.info(f"Sample executed with PID {pid}")
                
                # Wait for completion or timeout
                return_code = agent.sample_executor.wait(config.analysis_timeout)
                logger.info(f"Sample completed with return code: {return_code}")
